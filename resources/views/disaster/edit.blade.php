...
@php
   ...
   //dd(count($disaster_category))
   //dd($disaster_category[0]['question_id']);
...
<div class="format-container">
    ...
    <form  action="/disaster/{{$disaster['disater_id']}}" method="POST" id="disaster_edit"> 
        @csrf
        @method('PUT')
    </form>
        <div class="mb-10">
        <div class="div-input-create">
            ...
            <div class="input-div-70 inline-block ">
                <input form="disaster_edit" class="input" type="text" name="disaster_title" value="{{ old('disaster_title', $disaster['disater_tiltle']) }}">
    ...
    <div class="mb-10">
        <div class="div-input-create">
            ...
            <div class="input-div-70 inline-block ">
                @switch(old('status', $disaster['disater_status']))
                    @case(1)
                            @php
                                $ai1="checked"
                            @endphp 
                    ...
                    @case(0)
                        ...
                        @break
                @endswitch
                <input form="disaster_edit" type="radio" name="status" value="1" {{ $ai1 }}> 公開 
                <input form="disaster_edit" type="radio" name="status" value="0" {{ $ai0 }}> 非公開1111
            ...
            <table class="table" id="sortable">  
                ...
                <tbody class="connectedSortable"> 
                    ...
                    @if (old('checkbox_question_id') ) 
                     ...
                     <tr class="tr" >
                        ...
                        <td class="td text-center">
                            <input form="disaster_edit" name="checkbox_question_id[]" type="checkbox" value="{{ $values['question_id'] }}"
                    ...
                    <tr class="tr" >
                       ...
                       <td class="td text-center">
                           <input form="disaster_edit" name="checkbox_question_id[]" type="checkbox" value="{{ $values['question_id'] }}"
                               @for ($j=0; $j<count($disaster_category); $j++)
                                   @if ($disaster_category[$j]['question_id'] == $values['question_id'])
                                       {{ 'checked' }}
                                   @else
                                       {{ '' }}    
                                   @endif
                   ...
                   @endforeach
                    ...
                    {{-- @foreach ($question as $values)
                        <tr class="tr" >
                            <td class="num-row text-center"></td>
                            <td class="td text-center"><input form="disaster_edit" name="checkbox_question_id[]" type="checkbox" value="{{ $values['question_id'] }}"></td>
                            <td data-taskid="" class="taskSingleInline" class="td text-left">{{ $values["question_title"] }}</td>
                            @php
                                $number++;
                            @endphp
    ...
    <div class="mb-10">
        <div class="div-input-create">
            <div class="input-div-70 flex">
                ...
                <div class="div-b"><button style="width: 90px; height: 42px;"type="submit" form="disaster_edit" class="btn-big gray">保存</button></div>
            </div>
        </div>
    </div>
</div>
