...
<div class="format-container">
    ...
    <form id="my-form" action="/format" method="POST">
        @csrf
        <div class="mb-10">
        <div class="div-input-create">
            ...
            <div class="input-div-70 inline-block ">
                <textarea class="textarea" name="question_title"  id="" >{{ old('question_title') }}</textarea>
    ...
    <div id="create-radio" class="mb-10">
        <div class="div-input-create">
            ...
            <div class="input-div-70 inline-block ">
                ...
                @if(!old('question_typeof') || old('question_typeof')=='form')
                ...
                <div id="create-input" class="mb-10">
                    @switch(old('question_typeof'))
                        ...
                        @if(old('answer_title') && count(old('answer_title')) > 0)
                            @foreach (old('answer_title') as $checkbox)
                                <div class="mb-10" number='stt-{{ $i }}'>
                                    <input name="answer_title[]" value="{{ $checkbox }}" class="input count-input" type="text">
                                    @php
                                        if($i>1)
                                        {
                            ...
                            @endforeach
                            @else
                                {{-- <div class="mb-10"><input name="answer_title[]" value="" class="input" type="text"></div> --}}
                                {{-- <div class="mb-10"><input name="answer_title[]" value="" class="input" type="text"></div> --}}
                            @endif
                            @break
                        @case('radio')
                            @if(old('answer_title') && count(old('answer_title')) > 0)
                                @foreach (old('answer_title') as $radio)
                                    <div class="mb-10" number='stt-{{ $i }}'>
                                        <input name="answer_title[]" value="{{ $radio }}" class="input count-input" type="text"> 
                                        @php
                                        if($i>1)
                                        {
                                            $btn="<button style='background-color: #e884f8' id='remove-input' type='submit' class='btn'>削除</button>" ;
